...
public class RDBodyData implements RDData{
    ...
    public RDBodyData(List<RDLayer> layers, RDBoundingBox globalBoundingBox){
        ...
        for(RDLayer layer : layers){
            ...
            try{
                ...
                travelLogic(bodyDataStream, layer);
    ...
    private void travelLogic(ByteArrayOutputStream bodyDataStream, RDLayer layer) throws IOException{
        int relCounter = 0;
        RDPoint lastPoint = null;
        boolean travel = true;
        for(RDPoint point : layer.paths()){
